...
class DefiantParser:
    ...
    VOCABULARY_NOUNS = {"duplicates", "summary", "confirmation", "full", "last", "1.2m",
                        "issue_panel", "old", "x", "thirty", "bad_wrist", "correction",
                        "profile", "visibility", "tension", "reading_zoom"}
    ...
    def _require_declared_or_known(self, ident: str, line_num: int):
        cleaned = ident.strip('"')
        if cleaned not in self.declared and cleaned not in self.VOCABULARY_NOUNS:
